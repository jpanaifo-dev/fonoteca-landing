...
interface ServicesGridProps {
    content: Omit<typeof translations.es.species, 'items'> & {
        items: Array<{ title: string; count: string; icon?: string }>;
    };
}
...
export const ServicesGrid: React.FC<ServicesGridProps> = ({ content }) => {
    return (
        <section id="species" className="py-24 bg-light-bg dark:bg-[#0c141d]">
            <div className="container mx-auto px-6">
                <div className="mb-16">
                    <span className="text-accent-green font-bold tracking-widest uppercase text-sm mb-4 block">
                        {content.label}
                    </span>
                    <h2 className="text-4xl md:text-5xl text-primary-dark dark:text-white font-light">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {content.items.map((item, index) => (
                        <div key={index} className="group bg-white dark:bg-[#121b28] p-10 border border-gray-100 dark:border-gray-800 rounded-[2.5rem] hover:border-accent-green/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-accent-green/10 relative overflow-hidden">
                            ...
                            <h3 className="text-2xl text-primary-dark dark:text-white font-light mb-2 group-hover:text-accent-green transition-colors">
                                {item.title}
                            </h3>
